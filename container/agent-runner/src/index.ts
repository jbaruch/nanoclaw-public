...
async function runQuery(
  ...
  const mcpServersConfig = {
    ...
    // NANOCLAW_ONECLI_ENABLED=1 AND HTTPS_PROXY=... at spawn time. The
    // proxy env vars below are still forwarded so the MCP server's
    // outbound HTTPS routes through the gateway.
